...
<img src="{{ '/assets/images/themes/red-wings.png' | relative_url }}" alt="Detroit Red Wings Collection" class="set-hero-image">

<div id="collection-overview-redwings">
  <p>Loading collection overview...</p>
</div>

<div id="set-detail-redwings">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const overviewContainer = document.getElementById('collection-overview-redwings');
            ...
            complete: function(marketResults) {
              if (marketResults.data && marketResults.data.length > 0) {
                marketResults.data.forEach(row => {
                  if (row.Sport === 'Hockey' || row.Team === 'Detroit Red Wings') {
              ...
              overviewContainer.innerHTML = `
                <div class="collection-overview">
                  <div class="overview-title">Detroit Red Wings Collection Overview</div>
    ...
    const detailContainer = document.getElementById('set-detail-redwings');
    if (detailContainer) {
      Papa.parse(inventoryUrl, {
        download: true,
        header: true,
        ...
        complete: function(results) {
          ...
          const filteredData = results.data.filter(row => row.Sport === 'Hockey');

          if (filteredData.length === 0) {
            detailContainer.innerHTML = '<p>No Detroit Red Wings items in inventory.</p>';
            return;
          }
