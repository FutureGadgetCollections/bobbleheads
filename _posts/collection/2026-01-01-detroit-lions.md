...
<img src="{{ '/assets/images/themes/lions.jpg' | relative_url }}" alt="Detroit Lions Collection" class="set-hero-image">

<div id="collection-overview-lions">
  <p>Loading collection overview...</p>
</div>

<div id="set-detail-lions">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load overview stats
    const overviewContainer = document.getElementById('collection-overview-lions');
    if (overviewContainer) {
            ...
            complete: function(marketResults) {
              if (marketResults.data && marketResults.data.length > 0) {
                marketResults.data.forEach(row => {
                  if (row.Sport === 'Football' || row.Team === 'Detroit Lions') {
              ...
              overviewContainer.innerHTML = `
                <div class="collection-overview">
                  <div class="overview-title">Detroit Lions Collection Overview</div>
      ...
      function displayPartialOverview() {
        const gain = currentValue - costBasis;
        const gainPercent = costBasis > 0 ? (gain / costBasis * 100) : 0;
        const gainClass = gain >= 0 ? 'gain-positive' : 'gain-negative';
        const gainSign = gain >= 0 ? '+' : '';
    ...
    const detailContainer = document.getElementById('set-detail-lions');
    if (detailContainer) {
      Papa.parse(inventoryUrl, {
        download: true,
        header: true,
        ...
        complete: function(results) {
          ...
          const filteredData = results.data.filter(row => row.Sport === 'Football');

          if (filteredData.length === 0) {
            detailContainer.innerHTML = '<p>No Detroit Lions items in inventory.</p>';
            return;
          }
