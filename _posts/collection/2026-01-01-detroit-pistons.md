...
<img src="{{ '/assets/images/themes/pistons.png' | relative_url }}" alt="Detroit Pistons Collection" class="set-hero-image">

<div id="collection-overview-pistons">
  <p>Loading collection overview...</p>
</div>

<div id="set-detail-pistons">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const overviewContainer = document.getElementById('collection-overview-pistons');
            ...
            complete: function(marketResults) {
              if (marketResults.data && marketResults.data.length > 0) {
                marketResults.data.forEach(row => {
                  if (row.Sport === 'Basketball' || row.Team === 'Detroit Pistons') {
              ...
              overviewContainer.innerHTML = `
                <div class="collection-overview">
                  <div class="overview-title">Detroit Pistons Collection Overview</div>
    ...
    const detailContainer = document.getElementById('set-detail-pistons');
    if (detailContainer) {
      Papa.parse(inventoryUrl, {
        download: true,
        header: true,
        ...
        complete: function(results) {
          ...
          const filteredData = results.data.filter(row => row.Sport === 'Basketball');

          if (filteredData.length === 0) {
            detailContainer.innerHTML = '<p>No Detroit Pistons items in inventory.</p>';
            return;
          }
