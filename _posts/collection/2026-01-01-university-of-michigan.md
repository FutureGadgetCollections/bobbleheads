...
layout: single
permalink: /collection/university-of-michigan/
...
<img src="{{ '/assets/images/themes/michigan.png' | relative_url }}" alt="University of Michigan Collection" class="set-hero-image">

<div id="collection-overview-michigan">
  <p>Loading collection overview...</p>
</div>

<div id="set-detail-michigan">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const overviewContainer = document.getElementById('collection-overview-michigan');
            ...
            complete: function(marketResults) {
              if (marketResults.data && marketResults.data.length > 0) {
                marketResults.data.forEach(row => {
                  if (row.Sport === 'College' || row.Team === 'University of Michigan') {
              ...
              overviewContainer.innerHTML = `
                <div class="collection-overview">
                  <div class="overview-title">University of Michigan Collection Overview</div>
    ...
    const detailContainer = document.getElementById('set-detail-michigan');
    if (detailContainer) {
      Papa.parse(inventoryUrl, {
        download: true,
        header: true,
        ...
        complete: function(results) {
          ...
          const filteredData = results.data.filter(row => row.Sport === 'College');

          if (filteredData.length === 0) {
            detailContainer.innerHTML = '<p>No University of Michigan items in inventory.</p>';
            return;
          }
